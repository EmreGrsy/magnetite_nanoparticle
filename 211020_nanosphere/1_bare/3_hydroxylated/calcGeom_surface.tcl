...
topo readlammpsdata initial_structure_before_charge_redistribution.data
...
set center_sphere [measure center $all_atoms]

puts "sphere center is : $center_sphere"

set shell_fe [atomselect top "(type 2 3) and (sqrt(sqr(25-x) +sqr(25-y)+sqr(25-z)) > 9.5)"] 
#set fe_group [atomselect top "type 2 3"]
set num_fe_group [$shell_fe num]
...
foreach i3 [$shell_fe get index] {
	...
	if {[lindex [coord_structure $i3] 1] == 2} {
		if {[lindex [coord_structure $i3] 2] == 1} {
			...
			incr feTet_2c
			[atomselect top "index $i3"] set type 4
			[atomselect top "index $i3"] set resid 4
			[atomselect top "index $i3"] set resname T2

		} elseif {[lindex [coord_structure $i3] 2] == 3} {
                        incr feTet_3c
			[atomselect top "index $i3"] set type 5
			[atomselect top "index $i3"] set resid 5
			[atomselect top "index $i3"] set resname T3
		...
		if {[lindex [coord_structure $i3] 2] == 1} {
                        ...
                        incr feOct_3c 
			[atomselect top "index $i3"] set type 6
			[atomselect top "index $i3"] set resid 6
			[atomselect top "index $i3"] set resname O3

                } elseif {[lindex [coord_structure $i3] 2] == 4} {
                        incr feOct_4c 
			[atomselect top "index $i3"] set type 7
			[atomselect top "index $i3"] set resid 7
			[atomselect top "index $i3"] set resname O4

                } elseif {[lindex [coord_structure $i3] 2] == 5} {
                        incr feOct_5c 
			[atomselect top "index $i3"] set type 8
			[atomselect top "index $i3"] set resid 8
			[atomselect top "index $i3"] set resname O5
...
topo writelammpsdata surface_undercoordinated_fe_highlighted.data
#[atomselect top all] set resname NSP
[atomselect top all] writepdb undercoordinated.pdb
